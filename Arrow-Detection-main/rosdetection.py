...
def process_frame(frame, right_arrow, left_arrow):
    ...
    right_direction, right_deviation = match_and_annotate(frame, right_arrow, (0, 255, 0), "Right")
    left_direction, left_deviation = match_and_annotate(frame, left_arrow, (255, 0, 0), "Left")
...
class ArrowDetector(Node):
    ...
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(String, 'arrow_info', 10)
        self.timer = self.create_timer(0.1, self.timer_callback)

        log_initialization()
        self.right_arrow = cv2.imread("Arrow-Detection-main/Arrow-Detection-main/Right_Arrow.png", cv2.IMREAD_GRAYSCALE)
        self.left_arrow = cv2.imread("Arrow-Detection-main/Arrow-Detection-main/Left_arrow.png", cv2.IMREAD_GRAYSCALE)
    ...
    def timer_callback(self):
        ...
        if direction:
            msg = String()
            msg.data = f"{direction} arrow detected. Deviation from center: {deviation} pixels"
